# 3.1
设随机事件 $A, B$ 及其和事件 $A \cup  B$ 的概率分别是 0.4, 0.3, 0.6. 
若 $\bar{B}$ 表示 $B$ 的对立事件,那么积事件 $A\overline{B}$ 的概率 $P\left( {A\overline{B}}\right)  =$ _____.
...
设 $A, B$ 是任意两个随机事件, $A - B = A - {AB} = A\left( {\Omega  - B}\right)  = A\bar{B}$. 
事实上, 这是一个很容易理解的变形, 不妨按下列方式理解: $A - B$ 表示事件“ $A$ 发生 $B$ 不发生”, $A - {AB}$ 表示事件“在 $A$ 发生的事件中除掉 ${AB}$ 一起发生的事件”, $A\bar{B}$ 表示事件“ $A$ 发生 $B$ 不发生”, 很明显这三个事件是一样的.
...
(A) $P\left( C\right)  \leq  P\left( A\right)  + P\left( B\right)  - 1$ 
(B) $P\left( C\right)  \geq  P\left( A\right)  + P\left( B\right)  - 1$
(C) $P\left( C\right)  = P\left( {AB}\right)$ 
(D) $P\left( C\right)  = P\left( {A \cup  B}\right)$
