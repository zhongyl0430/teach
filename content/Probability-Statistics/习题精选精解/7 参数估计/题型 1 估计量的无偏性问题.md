# 2.1
已知总体 $X$ 的期望 ${EX} = 0$,方差 ${DX} = {\sigma }^{2},{X}_{1},\cdots,{X}_{n}$ 为其简单样本,均值为 $\bar{X}$,方差为 ${S}^{2}$. 则 ${\sigma }^{2}$ 的无偏估计量为_____.
...
由于
$$E\bar{X} = {EX} = 0,$$
$$E\left( {\bar{X}}^{2}\right)  = D\bar{X} + {\left( E\bar{X}\right) }^{2},$$
...
则 $E\left( {\frac{1}{2}n{\bar{X}}^{2} + \frac{1}{2}{S}^{2}}\right)  = {\sigma }^{2}$,
