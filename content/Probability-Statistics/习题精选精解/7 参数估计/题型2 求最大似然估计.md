...
$$
其中 $\lambda  > 0$ 是未知参数, $\alpha  > 0$ 是已知常数, 
根据来自总体 $X$ 的简单随机样本 ${X}_{1},{X}_{2},\cdots,{X}_{n}$, 
求 $\lambda$ 的最大似然估计量 $\widehat{\lambda }$.
...
$$
其中 $\theta  > 0$ 为未知参数,
又设 ${x}_{1},{x}_{2},\cdots,{x}_{n}$ 是 $X$ 的一组样本观测值,
求参数 $\theta$ 的最大似然估计值.
...
# 1.9
设 $X \sim  N\left( {\mu,{\sigma }^{2}}\right)$, ${x}_{1},{x}_{2},\cdots,{x}_{n}$ 为一组样本值, 
求参数 $\mu,{\sigma }^{2}$ 的极大似然估计.
...
$$\widehat{\mu } = \bar{X}$$
$$\hat{\sigma}^2 = \frac{1}{n}\mathop{\sum }\limits_{{i = 1}}^{n}{\left( {X}_{i} - \bar{X}\right) }^{2}$$
