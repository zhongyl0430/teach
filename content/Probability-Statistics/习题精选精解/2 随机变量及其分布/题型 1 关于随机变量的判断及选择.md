...
## 解 
因为 $\mathop{\sum }\limits_{{k = 1}}^{\infty }P\{ X = k\}  = \mathop{\sum }\limits_{{k = 1}}^{\infty }b{\lambda }^{k} = 1,\;{S}_{n} = \mathop{\sum }\limits_{{k = 1}}^{n}b{\lambda }^{k} = b \cdot  \frac{\left( {1 - {\lambda }^{n}}\right) \lambda }{1 - \lambda }$ 即
...
# 5.2
设 ${F}_{1}\left( x\right)$ 与 ${F}_{2}\left( x\right)$ 为两个分布函数, 其相应的概率密度 ${f}_{1}\left( x\right)$ 与 ${f}_{2}\left( x\right)$ 是连续函数, 则必为概率密度的是 (   )

(A) ${f}_{1}\left( x\right) {f}_{2}\left( x\right)$. 
(B) $2{f}_{2}\left( x\right) {F}_{1}\left( x\right)$.
(C) ${f}_{1}\left( x\right) {F}_{2}\left( x\right)$. 
(D) ${f}_{1}\left( x\right) {F}_{2}\left( x\right)  + {f}_{2}\left( x\right) {F}_{1}\left( x\right)$.

## 解 
因为 ${f}_{1}\left( x\right) {F}_{2}\left( x\right)  + {f}_{2}\left( x\right) {F}_{1}\left( x\right)  \geq  0$,且
...
(A) $k = \left( {n + 1}\right) p$ 
(B) $k = \left( {n + 1}\right) p - 1$ 
(C) $k = {np}$ 
(D) $k = \left\lbrack  {\left( {n + 1}\right) p}\right\rbrack$

## 解 
由二项分布的性质知, 应选 (D).

## 点评 
设 $X \sim  B\left( {n, p}\right)$,则使 $P\{ X = k\}$ 达到最大的 $k$,称为二项分布的最可能值,记为 ${k}_{0}$ 且
$$
...
# 5.5
当随机变量的可能值充满区间(   ),则 $\varphi \left( x\right)  = \cos x$ 可以成为随机变量 $X$ 的分布密度.
...
## 解 
由随机变量 $X$ 的分布密度函数 $\varphi \left( x\right)$ 的非负性可知 (B)、(C) 不该入选.
...
# 5.6
设 $X$ 为随机变量,若矩阵 $A = \left\lbrack  \begin{matrix} 2 & 3 & 2 \\  0 &  - 2 &  - X \\  0 & 1 & 0 \end{matrix}\right\rbrack$ 的特征值全为实数的概率为 0.5,
...
## 解 
由 $\left| {{\lambda E} - A}\right|  = \left\lbrack  \begin{matrix} \lambda  - 2 &  - 3 &  - 2 \\  0 & \lambda  + 2 & X \\  0 &  - 1 & \lambda  \end{matrix}\right\rbrack   = \left( {\lambda  - 2}\right) \left( {{\lambda }^{2} + {2\lambda } + X}\right)$,而其特征值全为实数的概率 $P\left\{  {{2}^{2} - {4X} \geq  0}\right\}   = P\{ X \leq  1\}  = {0.5}$,可见当 $X$ 服从 $\left\lbrack  {0,2}\right\rbrack$ 上均匀分布时成立.

故应选(A).

# 5.7
设随机变量 $X$ 的密度函数为 ${f}_{X}\left( x\right)$,则 $Y = 3 - {2X}$ 的密度函数为 (   ).
...
# 5.9
设随机变量 $X$ 服从正态分布 $N\left( {{\mu }_{1},{\sigma }_{1}^{2}}\right)$,随机变量 $Y$ 服从正态分布 $N\left( {{\mu }_{2},{\sigma }_{2}^{2}}\right)$,且
...
## 解 
$P\left\{  {\left| {X - {\mu }_{1}}\right|  < 1}\right\}   > P\left\{  {\left| {Y - {\mu }_{2}}\right|  < 1}\right\}$,即
...
# 5.10
设随机变量 $X$ 的分布函数 $F\left( x\right)  = \left\{  \begin{array}{ll} 0, & x < 0 \\  \frac{1}{2}, & 0 \leq  x < 1, \\  1 - {\mathrm{e}}^{-x}, & x \geq  1 \end{array}\right.$ 则 $P\{ X = 1\}  =$ (   )

(A) 0 (B) $\frac{1}{2}$ (C) $\frac{1}{2} - {\mathrm{e}}^{-1}$ (D) $1 - {\mathrm{e}}^{-1}$

## 解 
$P\{ X = 1\}  = P\{ X \leq  1\}  - P\{ X < 1\}  = F\left( 1\right)  - F\left( {1 - 0}\right)  = \left( {1 - {\mathrm{e}}^{-1}}\right)  - \frac{1}{2} = \frac{1}{2} - {\mathrm{e}}^{-1}$.
