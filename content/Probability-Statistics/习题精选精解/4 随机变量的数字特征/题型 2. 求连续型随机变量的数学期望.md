# 1.4
设在某一规定的时间间隔里, 某电气设备用于最大负荷的时间 $X$ (以分计) 是一个随机变量, 其概率密度为
$$
f\left( x\right)  = \left\{  \begin{array}{ll} \frac{1}{{\left( {1500}\right) }^{2}}x, & 0 \leq  x \leq  {1500} \\  \frac{-1}{{\left( {1500}\right) }^{2}}\left( {x - {3000}}\right), & {1500} < x \leq  {3000} \\  0, & \text{ 其他 } \end{array}\right.
$$

求 $E\left( X\right)$.

## 解
$$
\begin{aligned}
E\left( X\right)  &= {\int }_{-\infty }^{+\infty }{xf}\left( x\right) \mathrm{d}x \\
&= {\int }_{0}^{1500}\frac{{x}^{2}}{{\left( {1500}\right) }^{2}}\mathrm{\;d}x + {\int }_{1500}^{3000}\frac{-x}{{\left( {1500}\right) }^{2}}\left( {x - {3000}}\right) \mathrm{d}x \\
&= {1500} \text{ (分)}
\end{aligned}
$$
# 1.5
设随机变量 $X$ 的分布函数为
$$
F\left( x\right)  = \left\{  \begin{array}{ll} 1 - \frac{4}{{x}^{2}}, & x \geq  2 \\  0, & x < 2 \end{array}\right.
$$

求 $X$ 的期望.

## 解
因为 $X$ 的概率密度为
$$
f\left( x\right)  = {F}^{\prime }\left( x\right)  = \left\{  \begin{array}{ll} \frac{8}{{x}^{3}}, & x \geq  2 \\  0, & x < 2 \end{array}\right.
$$
所以 
$$
\begin{aligned}
E\left( X\right)  &= {\int }_{-\infty }^{+\infty }{xf}\left( x\right) \mathrm{d}x \\
&= {\int }_{2}^{+\infty }\frac{8}{{x}^{2}}\mathrm{\;d}x \\
&= 4
\end{aligned}
$$

# 1.6
设 $f\left( x\right)$ 为随机变量 $X$ 的密度函数, 若对于常数 $c$, 有
$$
f\left( {c + x}\right)  = f\left( {c - x}\right),\;x > 0
$$
且 ${EX}$ 存在,
试证明 ${EX} = c$.

## 证 
由数学期望的定义知
$$
\begin{aligned}
{EX} &= {\int }_{-\infty }^{+\infty }{xf}\left( x\right) \mathrm{d}x \\
&\xrightarrow{x = c + t} {\int }_{-\infty }^{+\infty }\left( {c + t}\right) f\left( {c + t}\right) \mathrm{d}t \\
&= {\int }_{-\infty }^{+\infty }{cf}\left( {c + t}\right) \mathrm{d}t + {\int }_{-\infty }^{+\infty }{tf}\left( {c + t}\right) \mathrm{d}t
\end{aligned}
$$
而
$$
\begin{aligned}
{\int }_{-\infty }^{+\infty }{cf}\left( {c + t}\right)  &= c{\int }_{-\infty }^{+\infty }f\left( {c + t}\right) \mathrm{d}t \\
&\xrightarrow{x = \left( {c + t}\right) } c{\int }_{-\infty }^{+\infty }f\left( x\right) \mathrm{d}x \\
&= c
\end{aligned}
$$
由题意知:
$$
\begin{aligned}
{\int }_{-\infty }^{0}{tf}\left( {c + t}\right) \mathrm{d}t &= {\int }_{-\infty }^{0}{tf}\left( {c - t}\right) \mathrm{d}t \\
&\xrightarrow{u =  - t} {\int }_{0}^{+\infty }{uf}\left( {c + u}\right) \mathrm{d}u \\
&=  - {\int }_{0}^{+\infty }{tf}\left( {c + t}\right) \mathrm{d}t
\end{aligned}
$$
即
$$
{\int }_{-\infty }^{0}{tf}\left( {c + t}\right) \mathrm{d}t + {\int }_{0}^{+\infty }{tf}\left( {c + t}\right) \mathrm{d}t = 0
$$

亦即
$$
{\int }_{-\infty }^{+\infty }{tf}\left( {c + t}\right) \mathrm{d}t = 0
$$
从而证明 ${EX} = c$.