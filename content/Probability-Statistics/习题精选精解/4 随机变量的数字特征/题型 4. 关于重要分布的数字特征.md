...
# 4. 39
设 $X \sim  P\left( {16}\right), Y \sim  E\left( 2\right),{\rho }_{XY} =  - {0.5}$,则 $\operatorname{Cov}\left( {X, Y + 1}\right)  =$ _____, $E\left( {{Y}^{2} + {XY}}\right)$ $=$ _____, $D\left( {X - {2Y}}\right)  =$ _____.
...
# 4.40
设一物体是圆截面,测量其直径,设其直径 $X$ 服从 $\left\lbrack  {0,3}\right\rbrack$ 上的均匀分布,则求横截面积 $Y$ 的数学期望和方差.
