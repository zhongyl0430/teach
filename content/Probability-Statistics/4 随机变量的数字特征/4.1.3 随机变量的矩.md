...
如果 $E\left\lbrack  {\left( X - E\left\lbrack  X\right\rbrack  \right) }^{3}\right\rbrack   >$ 0,则 $X$ 的分布取值大于 $E\left\lbrack  X\right\rbrack$ 的概率较大,此时称 $X$ 的分布为右偏. 
如果 $E\left\lbrack  {\left( X - E\left\lbrack  X\right\rbrack  \right) }^{3}\right\rbrack   < 0$,则称 $X$ 的分布为左偏. 
