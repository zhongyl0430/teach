- 已知 ^setting
	- 某厂生产的零件尺寸 $X \sim  N\left( {{12.5},{\sigma }^{2}}\right)$. 
...
和 $\alpha  = {0.05}$,
查表或用 $\mathrm{R}$ 软件计算的
- ${\chi }_{0.975}^{2}\left( 4\right)  = {11.14329}$ 
- ${\chi }_{0.025}^{2}\left( 4\right)  = 0.4844186$,

用 ![[3. 方差的区间估计 (已知期望)#^eq-7-3-7]]
得到 ${\sigma }^{2}$ 的置信度为 0.95 的区间估计 $\left\lbrack  {{0.13},{2.89}}\right\rbrack$.
