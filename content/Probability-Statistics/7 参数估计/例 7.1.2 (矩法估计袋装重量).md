长期的生产经验告诉我们, 水泥厂成品打包机装袋的重量服从正态分布, 
试用矩法估计来估计一台打包机装袋的重量的均值和标准差.

## 解 
设装袋的重量为 $X$,样本为 $\left( {{X}_{1},{X}_{2},\cdots,{X}_{n}}\right)$,
由题设 $X \sim  N\left( {\mu,{\sigma }^{2}}\right)$ 和 $E\left\lbrack  X\right\rbrack   = \mu,\operatorname{Var}\left\lbrack  X\right\rbrack   = {\sigma }^{2}$, 即
$$
