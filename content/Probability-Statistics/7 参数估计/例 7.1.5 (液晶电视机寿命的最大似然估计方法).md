据经验, 进入稳态期的生产线生产出的液晶电视机的使用寿命服从指数分布, 
试用最大似然估计方法估计该厂生产线生产出的液晶电视机的平均寿命.

## 解 
由题意设 $X \sim  \operatorname{Exp}\left( \lambda \right)$, 样本观测值为 $\left( {{x}_{1},{x}_{2},\cdots,{x}_{n}}\right)$, 
当 ${x}_{i} > 0(i =$ $1,2,\cdots, n)$ 时, $X$ 的似然函数为
$$
...
$$
也就是, 该厂生产线生产出的液晶电视机的平均寿命的最大似然估计量为 $1/ \bar{X}$.