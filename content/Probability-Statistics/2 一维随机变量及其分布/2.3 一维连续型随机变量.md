...
由 (2.1.3) 立刻得到
$$
P\left( {a < X \leq  b}\right)  = {F}_{X}\left( b\right)  - {F}_{X}\left( a\right)  = {\int }_{a}^{b}{f}_{X}\left( x\right) \mathrm{d}x, \;\forall a < b \in  \mathbb{R}. \tag{2.3.4}
$$
...
这表明*连续型随机变量取任意单点值的概率为零*, 这一点与离散型随机变量截然不同. 
...
- [[例 2.3.1 (分布密度函数的求解)]]
	- 例 2.3.1 中的分布通常称为**标准柯西**分布. 
	- 类似的 [[题型2 概率密度与分布函数的转化以及求概率#3.6]]
- [[题型 1 概率密度的性质#3.3]]
- [[题型2 概率密度与分布函数的转化以及求概率#3.5]]
## 应用
