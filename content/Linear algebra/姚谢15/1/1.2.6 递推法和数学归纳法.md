...
递推法的常用步骤是: 按行或列展开行列式, 使行列式降阶, 比较原行列式和降阶后的行列式的异同, 找出递推关系. 如降阶一次仍看不出关系, 可再降一次试试. 从递推式求通式往往需要一定的技巧, 如下面的例 1.23 和例 1.25 都需要相当的技巧. 读者可细心体会之. 数学归纳法也是一种常用的方法, 本质上也是一种递推法. 许多问题用数学归纳法证明往往比较简单, 但它必须事先知道结论. 因此有时可以先猜出结论, 然后归纳地证明它.
...
$$
{D}_{n} = \left| \begin{matrix} a & b & & & & \\ c & a & b & & & \\ \cdot & c & a & b & & \\ & & \ddots & \ddots & \ddots & \\ & & & c & a & b \\ & & & & c & a \end{matrix}\right| .
$$
...
$$
{D}_{n} - \alpha {D}_{n - 1} = \beta \left( {{D}_{n - 1} - \alpha {D}_{n - 2}}\right) ,{D}_{n} - \beta {D}_{n - 1} = \alpha \left( {{D}_{n - 1} - \beta {D}_{n - 2}}\right) .
$$
...
其中 ${D}_{n}$ 是形如例 1.23 的行列式, $a = 2\cos x, b = c = 1$ . 根据例 1.23 的结论,我们可以事先解出: $\alpha = \cos x + \mathrm{i}\sin x,\beta = \cos x - \mathrm{i}\sin x$ .
