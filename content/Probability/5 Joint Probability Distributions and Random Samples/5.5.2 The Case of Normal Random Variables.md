...
> [!proposition]
> If ${X}_{1},{X}_{2},\ldots ,{X}_{n}$ are independent, normally distributed rv’s (with possibly different means and/or variances), then any linear combination of the ${X}_{i}$ ’s also has a normal distribution. 
> In particular, the difference ${X}_{1} - {X}_{2}$ between two independent, normally distributed variables is itself normally distributed.
