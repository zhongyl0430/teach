...
# 77
A health-food store stocks two different brands of a certain type of grain. Let $X =$ the amount (lb) of brand A on hand and $Y =$ the amount of brand $\mathrm{B}$ on hand. Suppose the joint pdf of $X$ and $Y$ is
...
# 80
The mean weight of luggage checked by a randomly selected tourist-class passenger flying between two cities on a certain airline is ${40}\mathrm{{lb}}$ , and the standard deviation is ${10}\mathrm{{lb}}$ . The mean and standard deviation for a business-class passenger are ${30}\mathrm{{lb}}$ and $6\mathrm{{lb}}$ , respectively.
...
# 83
Let $\mu$ denote the true $\mathrm{{pH}}$ of a chemical compound. A sequence of $n$ independent sample $\mathrm{{pH}}$ determinations will be made. Suppose each sample $\mathrm{{pH}}$ is a random variable with expected value $\mu$ and standard deviation .1 . How many determinations are required if we wish the probability that the sample average is within .02 of the true $\mathrm{{pH}}$ to be at least .95 ? What theorem justifies your probability calculation?
...
# 85
Refer to Exercise 58, and suppose that the ${X}_{i}$ ’s are independent with each one having a normal distribution. What is the probability that the total volume shipped is at most ${100},{000}{\mathrm{{ft}}}^{3}$ ?

# 86
A student has a class that is supposed to end at 9:00 A.M. and another that is supposed to begin at $9 : {10}$ A.M.
...
# 87
Garbage trucks entering a particular waste-management facility are weighed prior to offloading their contents. Let $X =$ the total processing time for a randomly selected truck at this facility (waiting, weighing, and offloading). The article "Estimating Waste Transfer Station Delays Using GPS" (Waste Mgmt., 2008: 1742-1750) suggests the plausibility of a normal distribution with mean ${13}\mathrm{\;{min}}$ and standard deviation $4\mathrm{\;{min}}$ for $X$ . Assume that this is in fact the correct distribution.
...
# 90
Suppose a randomly chosen individual’s verbal score $X$ and quantitative score $Y$ on a nationally administered aptitude examination have a joint pdf
...
# 92
a. Show that $\operatorname{Cov}\left( {X, Y + Z}\right) = \operatorname{Cov}\left( {X, Y}\right) + \operatorname{Cov}\left( {X, Z}\right)$ .
...
# 93
A rock specimen from a particular area is randomly selected and weighed two different times. Let $W$ denote the actual weight and ${X}_{1}$ and ${X}_{2}$ the two measured weights. Then ${X}_{1} = W + {E}_{1}$ and ${X}_{2} = W + {E}_{2}$ , where ${E}_{1}$ and ${E}_{2}$ are the two measurement errors. Suppose that the ${E}_{i}$ ’s are independent of one another and of $W$ and that $V\left( {E}_{1}\right) = V\left( {E}_{2}\right) = {\sigma }_{E}^{2}$ .
...
# 96
A more accurate approximation to $E\left\lbrack {h\left( {{X}_{1},\ldots ,{X}_{n}}\right) }\right\rbrack$ in Exercise 95 is
...
# 97
Let $X$ and $Y$ be independent standard normal random variables, and define a new rv by $U = {.6X} + {.8Y}$ .
