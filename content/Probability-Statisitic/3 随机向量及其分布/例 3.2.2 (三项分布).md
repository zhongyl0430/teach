设随机试验只有 $A, B$ 和 $C$ 等三个结果. 各结果出现的概率分别为 $p, q$ 和 $1 - p - q$. 现将该随机试验独立地做 $n$ 次,记 $X$ 和 $Y$ 分别为 $n$ 次试验中 $A$ 和 $B$ 发生的次数,试求(X, Y)的联合分布和边缘分布.

## 解 
由题意 $X$ 和 $Y$ 的可能取值为 $0,1,2,\cdots, n$. 由于试验是独立的. 按独立试验概型来计算得
