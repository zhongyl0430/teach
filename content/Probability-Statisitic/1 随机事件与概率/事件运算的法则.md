前面我们定义了事件的积 (交)、和 (并)、差及对立等运算, 与算术运算和极限运算等类似, 为分析计算较为复杂事件的概率, 我们还需熟悉事件运算的法则. 
这些运算法则, 读者在 [[例 1.1.6 (三个事件的表达)]] 和 [[例 1.1.7 (飞碟射击)]] 中已经有所体会了.
...
### 对偶律
对于积 (交)、和 (并) 及对立运算还有对偶律：
因为 $\overline{A \cup  B}$ 表示 “不是 $A$ 和 $B$ 至少发生一个”, 亦即 $A$ 和 $B$ 都不发生, 所以有 $$\overline{A \cup  B} = \bar{A} \cap  \bar{B} \tag{1.1.6}$$同理, 可以说明 $$\overline{A \cap  B} = \bar{A} \cup  \bar{B} \tag{1.1.7}$$
